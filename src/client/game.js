...
function tick(event) {
    ...
    if (Key.isDown(Key.W)) {
        ...
        if (checkCollisionWithBuildings(playerGetPos(0, displacement)[0], playerGetPos(0, displacement)[1], radius))
            if (checkPlayerCollisionWithMonsters(playerGetPos(0, displacement)[0], playerGetPos(0, displacement)[1], radius))
    ...
    if (Key.isDown(Key.A)) {
        ...
        if (checkCollisionWithBuildings(playerGetPos(-displacement, 0)[0], playerGetPos(-displacement, 0)[1], radius))
            if (checkPlayerCollisionWithMonsters(playerGetPos(-displacement, 0)[0], playerGetPos(-displacement, 0)[1], radius))
...
function checkMonsterCollisionWithPlayer(x,y){
    if (Math.pow(playerGetPos()[0]-x, 2)+Math.pow(playerGetPos()[1]-y, 2)<=Math.pow(monsterRadius+radius+personalSpace, 2))
        return false;
    return true;
}
